...
from target_tiller.balancers import BuyOnlyBalancer
from target_tiller.input import TargetsLoaderYAML, TDPortfolioLoaderCSV


class TestBuyOnlyBalancer:
    ...
    def test_init_options(self):
        ...
        balancer = BuyOnlyBalancer(portfolio=portfolio,
                                   targets=targets,
        ...
        with pytest.raises(ValueError, match=r".*portfolio.*must.*provided.*"):
            balancer = BuyOnlyBalancer(targets=targets,
                                       transactions_config=transactions_config)
        ...
        with pytest.raises(ValueError, match=r".*wrong.*class.*"):
    ...
    def test_all1(self):
        ...
        assert balancer.next_total_value(["WTF", "LOL", "HUH"]) == 100000.0
        ...
        multiplier = balancer.multiplier(["LOL", "WTF"])
        assert (multiplier == 4.0 / 3.0)

        wtf_weight = 0.4 * multiplier
        lol_weight = 0.35 * multiplier

        # Next value of only the two holdings we are buying
        assert balancer.next_total_value(["WTF", "LOL"]) == 70000.0

        # Should "WHY" have been copied over too?
        assert next_portfolio.dump_holdings() == {"WTF": 70000.0 * wtf_weight,
                                                  "LOL": 70000.0 * lol_weight,
                                                  "HUH": 30000.0}

        assert round(next_portfolio.holding("WTF").value, 5) == 37333.33333
        assert round(next_portfolio.holding("LOL").value, 5) == 32666.66667

        # Not perfect balance ...
        assert portfolio.balance(targets) < 0.8
        assert next_portfolio.balance(targets) > 0.93
    ...
    def test_all2(self):
        ...
        # We are spending 6000.0, so ...
        assert balancer.next_total_value(["WTF", "LOL", "NGL"]) == 10000.0
        ...
        assert next_portfolio.dump_holdings() == {"WTF": 4000.0,
                                                  "LOL": 5000.0,
                                                  "NGL": 1000.0}
