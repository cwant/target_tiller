...
from target_tiller.balancers import SellOnlyBalancer
from target_tiller.input import TargetsLoaderYAML, TDPortfolioLoaderCSV


class TestSellOnlyBalancer:
    ...
    def test_init_options(self):
        ...
        balancer = SellOnlyBalancer(portfolio=portfolio,
                                    targets=targets,
        ...
        with pytest.raises(ValueError, match=r".*portfolio.*must.*provided.*"):
            balancer = SellOnlyBalancer(targets=targets,
                                        transactions_config=transactions_config)
    ...
    def test_all2(self):
        ...
        assert next_portfolio.dump_holdings() == {"WTF": 800.0,
                                                  "LOL": 1000.0,
                                                  "NGL": 0.0}
        ...
        # Not perfect balance ...
        assert portfolio.balance(targets) < 0.8
        assert next_portfolio.balance(targets) > 0.85
